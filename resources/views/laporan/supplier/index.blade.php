...
<div class="glass-card">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="page-title mb-0">Laporan Data Supplier</h4>

        <div class="d-flex gap-2">
            <a href="{{ url('/laporan/supplier/pdf?' . request()->getQueryString()) }}"
               class="btn btn-danger btn-sm rounded-3">
                Export PDF
            </a>
            <a href="{{ url('/laporan/supplier/excel?' . request()->getQueryString()) }}"
        ...
        <div class="row g-2 align-items-end">
            ...
            <div class="col-md-7">
                ...
                <input type="text" name="search"
                       class="form-control"
                       placeholder="Kode / Nama / Alamat / Telepon Supplier"
    ...
    <div class="table-responsive">
        <table class="table align-middle">
            <thead>
                <tr>
                    <th>{!! sortLink('Kode Sup', 'kd_sup') !!}</th>
                    <th>{!! sortLink('Nama Supplier', 'nm_sup') !!}</th>
                    <th>{!! sortLink('Alamat', 'alamat') !!}</th>
                    <th>{!! sortLink('Kota', 'kota') !!}</th>
                    <th>{!! sortLink('Kode Pos', 'kd_pos') !!}</th>
                    <th>{!! sortLink('Telepon', 'phone') !!}</th>
            ...
            <tbody>
                @forelse($data as $d)
                <tr>
                    <td class="fw-bold text-center">{{ $d->kd_sup }}</td>
                    <td>{{ $d->nm_sup }}</td>
                    <td>{{ $d->alamat }}</td>
                    <td>{{ $d->kota }}</td>
