...
<div class="glass-card">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="page-title mb-0">Laporan Retur Penjualan</h4>
        <div class="d-flex gap-2">
            <a href="{{ route('laporan.retur.penjualan.pdf', request()->all()) }}"
               class="btn btn-danger btn-sm">
                Export PDF
            </a>
            <a href="{{ route('laporan.retur.penjualan.excel', request()->all()) }}"
            ...
            @forelse($retur as $r)
                @foreach($r->detail as $d)
                    ...
                    <tr>
                        ...
                        <td>
                            {{ $r->jual->konsumen->nm_kons ?? '-' }}
                        </td>
                        <td>
                            {{ $d->barang->nm_brg ?? $d->kd_brg }}
                        </td>
                        <td class="text-center fw-bold">
                            {{ $d->qty_retur }}
                        </td>
